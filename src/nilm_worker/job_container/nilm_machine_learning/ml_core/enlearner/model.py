...
class CNN1DModel(BaseModel):
    def create(self):
        ...
        #adam = Adam(lr=0.0000005, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
        ...
        #adam = Adam(lr=1e-6) #, epsilon=1e-10)
        #self.model.compile(optimizer='rmsprop',loss='categorical_crossentropy') #,metrics=['accuracy'])
        #sgd = Nadam(lr=0.0002) #Adamax(lr=0.0002) #Adagrad(lr=0.0001) #SGD(lr=0.00001) #, decay=1e-6, momentum=0.9, nesterov=True)
...
class CNN1DSimple(CNN1DModel):
    def create(self):
        ...
        adam = Adam(lr=1e-7) #, epsilon=1e-10)
        self.model.compile(optimizer='rmsprop',loss='categorical_crossentropy',metrics=['accuracy'])
        #adam = Adam(lr=0.000000001, beta_1=0.9, beta_2=0.999, epsilon=1e-14)

...
class EnCompactForm(DataFormat):
    ...
    def read_raw_data(self, filename):
        ...
        #print 'onoff at:' ,self._idx_onoff
        self.raw_data = pd.DataFrame()

        import hashlib
        import os.path
        data_cached = hashlib.md5( "".join(filename).encode() ).hexdigest()

        if 'data_tmp_dir' in self.opt and os.path.isfile(self.opt['data_tmp_dir']+'/'+data_cached):
            self.logger.debug( 'read from cache')
            self.raw_data =  feather.read_dataframe(self.opt['data_tmp_dir'] + '/' + data_cached)
        else:
            ...
            if 'data_tmp_dir' in self.opt:
                feather.write_dataframe(self.raw_data,self.opt['data_tmp_dir'] +'/'+data_cached)
                ...
                if count_sec == 0:
                    if temp_buffer_ap1 is not None and len(temp_buffer_ap1) == 600 :
                        try:
                            ...
                            self.logger.debug('error in file reading => ' + serial + ',' + timestamp)

		...
		while self._runnable:
			try:
				...
				if not DataSender(self._logger, workerObj).sendMsg(jobMessage):
					...
					self._resourceHandler.changeStatusToIdelJobDAG(jobId, taskId, workerId = cvtWorkerId(workerObj))
				else:
					self._resourceHandler.setIdelToRunningCpu(workerObj, processType)
					self._resourceHandler.changeStatusToRunJobDAG(jobId, taskId, workerId = cvtWorkerId(workerObj))
			except Exception, e:
				self._logger.exception(e)
			self._printResourceStatus()
		...
		while self._runnable:
			try:
				...
				if not DataSender(self._logger, workerObj).sendMsg(jobMessage):
					...
					self._resourceHandler.changeStatusToIdelJobDAG(jobId, taskId)
				else:
					self._resourceHandler.setIdelToRunningCpu(workerObj, processType)
					self._resourceHandler.changeStatusToRunJobDAG(jobId, taskId)
			except Exception, e:
				self._logger.exception(e)
			self._printResourceStatus()
			self._resourceHandler.getTasksOnJobDAG(jobId)

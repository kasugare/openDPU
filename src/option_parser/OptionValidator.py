...
from common.conf_nilm_groups import getNilmGroupTable
from common.conf_system import getSystemEnv, getMaxProcess
	...
	def _checkSiteId(self):
		...
		for sid in sids:
			if isNumber(sid):
				...
				if sid == '*':
					gids = []
					groupInfos = getNilmGroupTable(self._logger)
					groupNames = [str(unicode(groupName)) for groupName in self._userOptions.get('groups')]
					if not groupNames:
						gids = [groupInfos[groupName] for groupName in groupInfos.keys()]
					else:
						gids = []
						for groupName in groupNames:
							if groupInfos.has_key(groupName):
								gids.append(groupInfos['groupName'])
					sids = self._serviceManager.getNilmTargetSids(gids)
					return sids
				else:

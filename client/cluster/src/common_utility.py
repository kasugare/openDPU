...
def getMailInfo(confName = 'MAIL'):
	conf = _getConfig()
	from_mail_addr = conf.get(confName, 'from_mail_addr')
	to_mail_addrs = conf.get(confName, 'to_mail_addrs')
	mail_lib = conf.get(confName, 'mail_lib')
	return from_mail_addr, to_mail_addrs, mail_lib

def htime(timestamp, tz='Asia/Seoul'):
	date = datetime.datetime.fromtimestamp(timestamp/1e3, pytz.timezone(tz)).strftime('%Y-%m-%d %H:%M:%S')
	return date

def hdate(timestamp, tz='Asia/Seoul'):
